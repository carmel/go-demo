...
func testNonceSafety(t *testing.T, c elliptic.Curve, tag string) {
	...
	hashed := []byte("testing")
	r0, s0, err := ecdsa.Sign(zeroReader, priv, hashed)
	if err != nil {
		t.Errorf("%s: error signing: %s", tag, err)
		return
	}
	...
	hashed = []byte("testing...")
	r1, s1, err := ecdsa.Sign(zeroReader, priv, hashed)
	if err != nil {
		t.Errorf("%s: error signing: %s", tag, err)
		return
	}
...
func testINDCCA(t *testing.T, c elliptic.Curve, tag string) {
	...
	hashed := []byte("testing")
	r0, s0, err := ecdsa.Sign(rand.Reader, priv, hashed)
	if err != nil {
		t.Errorf("%s: error signing: %s", tag, err)
		return
	}
...
func TestGenRSA(t *testing.T) {
	...
	if err == nil {
		...
		block := pem.Block{
			Type:  "rsa private key header",
			Bytes: x509PrivateKey,
		}
		//保存到文件中
		privateFile, _ := os.Create("rsaPri.pem")
		pem.Encode(privateFile, &block)
		defer privateFile.Close()
	} else {
		fmt.Println(err)
	...
	if err == nil {
		block := pem.Block{
			Type:  "rsa public key header",
			Bytes: x509PublicKey,
		}
		//保存到文件中
		publicFile, _ := os.Create("rsaPub.pem")
